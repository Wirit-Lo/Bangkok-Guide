...
// --- (Existing components like ImageLightbox, StarRatingInput, ReviewCard, ProductModal are assumed to be here and are unchanged) ---
...
const ReviewCard = ({ review, currentUser, onReviewDeleted, onEditClick }) => {
    ...
    const handleToggleLike = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/reviews/${review.id}/toggle-like`, {
                method: 'POST',
                headers: { 
            ...
            if (response.ok) setLikes(data.likesCount);
            else { setUserHasLiked(review.user_has_liked); setLikes(review.likes_count); }
        } catch (error) { setUserHasLiked(review.user_has_liked); setLikes(review.likes_count); }
    };

    const fetchComments = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/reviews/${review.id}/comments`);
            const data = await response.json();
            setComments(data);
            ...
            <div className="flex justify-between items-start">
                <div>
                    <div className="flex items-center mb-2">
                        ...
                        <div className="flex">{[...Array(5)].map((_, i) => (<Star key={i} size={16} fill={i < review.rating ? '#facc15' : '#e5e7eb'} className={i < review.rating ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'} strokeWidth={0} />))}</div>
                    </div>
            ...
            {review.image_urls && review.image_urls.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                    {review.image_urls.map((imgUrl, index) => (
                        <img key={index} src={imgUrl} alt={`Review ${index + 1}`} className="w-24 h-24 object-cover rounded-md" />
                    ))}
                </div>
            )}
            <div className="flex items-center space-x-4 mt-3 text-sm text-gray-500 dark:text-gray-400">
                <button onClick={handleToggleLike} className={`flex items-center space-x-1 hover:text-blue-600 dark:hover:text-blue-400 ${userHasLiked ? 'text-blue-600 dark:text-blue-400 font-bold' : ''}`}><ThumbsUp size={16} /><span>{likes} ถูกใจ</span></button>
...
const ProductModal = ({ product, locationId, onClose, onSave, setNotification, handleAuthError }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const url = isEditing ? `http://localhost:5000/api/products/${product.id}` : 'http://localhost:5000/api/products';
...
const DetailPage = ({ item, setCurrentPage, onReviewSubmitted, handleItemClick, currentUser, favorites, handleToggleFavorite, handleEditItem, setNotification, handleAuthError }) => {
    ...
    useEffect(() => {
        if (item && item.category) {
            ...
            const fetchSimilar = async () => {
                try {
                    const response = await fetch(`http://localhost:5000/api/locations/same-category?category=${encodeURIComponent(item.category)}&excludeId=${item.id}`);
    ...
    const fetchLocationProducts = useCallback(async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/locations/${item.id}/products`);
    ...
    const fetchReviews = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/reviews/${item.id}${userIdQuery}`);
    ...
    const handleReviewImageChange = (e) => {
        if (e.target.files) setNewReviewImages(prev => [...prev, ...Array.from(e.target.files)].slice(0, 5));
    };
    ...
    const handleReviewSubmit = async (e) => {
        e.preventDefault();
        if (newRating === 0) return alert('กรุณาให้คะแนนดาวก่อนส่งรีวิว');
        ...
        formData.append('locationType', item.type);
        ...
        try {
            ...
            setNotification({ message: 'เกิดข้อผิดพลาดในการส่งรีวิว', type: 'error' });
        } 
    ...
    const handleDeleteReview = async (reviewId) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/reviews/${reviewId}`, {
                ...
                body: JSON.stringify({ locationId: item.id, locationType: item.type }),
            });
            if (response.status === 401 || response.status === 403) return handleAuthError();
            if (!response.ok) {
                const errData = await response.json();
                throw new Error(errData.error || 'ไม่สามารถลบรีวิวได้');
            }
    ...
    const handleEditClick = (review) => {
        ...
        setEditedImages(review.image_urls.map(url => ({ type: 'existing', data: url })));
    };
    ...
    const handleEditImageChange = (e) => {
        if (e.target.files) {
            ...
            setEditedImages(prev => [...prev, ...newFiles].slice(0, 5));
        }
    };
    ...
    const handleUpdateReview = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                const errData = await response.json();
                throw new Error(errData.error || 'ไม่สามารถอัปเดตรีวิวได้');
            }
    ...
    const renderStars = (rating) => {
        ...
        return (<><svg width="0" height="0" className="absolute"><defs><linearGradient id="half-gradient"><stop offset="50%" stopColor="#facc15" /><stop offset="50%" stopColor="#e5e7eb" /></linearGradient></defs></svg>{[...Array(full)].map((_, i) => <Star key={`f-${i}`} size={20} fill="#facc15" stroke="#facc15" />)}{half && <Star key="h" size={20} fill="url(#half-gradient)" stroke="#facc15" />}{[...Array(empty)].map((_, i) => <Star key={`e-${i}`} size={20} fill="#e5e7eb" stroke="#9ca3af" />)}</>);
    };
    ...
    const handleDeleteProduct = async (productId) => {
        if (!window.confirm('คุณแน่ใจหรือไม่ว่าต้องการลบของขึ้นชื่อนี้?')) return;
        const token = localStorage.getItem('token');
        if (!token) return handleAuthError();
    ...
    const handleRequestDeletion = async () => {
        if (!window.confirm('คุณแน่ใจหรือไม่ว่าต้องการส่งคำขอลบสถานที่นี้? Admin จะทำการตรวจสอบคำขอของคุณ')) return;
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/locations/${item.id}/request-deletion`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            setTimeout(() => {
                setCurrentPage(item.type === 'attraction' ? 'attractions' : 'foodshops');
            }, 2000);
    ...
    if (!item) {
      return <div className="text-center py-20 dark:text-gray-300">กำลังโหลดข้อมูลสถานที่...</div>;
    }
                ...
                <div className="flex flex-col lg:flex-row lg:gap-12">
                    ...
                    <main className="lg:w-2/3 w-full">
                        {item.status === 'pending_deletion' && (
                            <div className="p-4 mb-6 bg-yellow-100 dark:bg-yellow-900/30 border-l-4 border-yellow-500 text-yellow-800 dark:text-yellow-200 rounded-r-lg">
                                <p className="font-bold">สถานะ: รอการอนุมัติลบ</p>
                                <p>สถานที่นี้ถูกส่งคำขอลบแล้ว และกำลังรอการตรวจสอบจากผู้ดูแลระบบ</p>
                            </div>
                        )}
                        ...
                        <div className="p-4 sm:p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl animate-fade-in-up">
                            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {isOwner && item.status === 'approved' && (
                                        <>
                                            <button onClick={() => handleEditItem(item)} className="flex items-center gap-2 px-4 py-2 border rounded-full text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                                <Edit size={16} /> <span className="font-semibold">แก้ไข</span>
                                            </button>
                                            <button 
                                            ...
                                            >
                                                <Trash2 size={16} /> <span className="font-semibold">{isRequestingDelete ? 'กำลังส่งคำขอ...' : 'ร้องขอลบ'}</span>
                                            </button>
                                        </>
                                    )}
                                </div>
                                    ...
                                    <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentSlide * 100}%)` }}>
                                        {allImages.map((img, index) => (
                                            <img 
                                                ...
                                                onError={(e) => { e.target.src = 'https://placehold.co/800x450/cccccc/333333?text=Image+Not+Found'; }}
                                            />
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    <>
                                        ...
                                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2">
                                            {allImages.map((_, index) => (
                                                <button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full transition-colors ${currentSlide === index ? 'bg-white' : 'bg-white/50 hover:bg-white'}`}></button>
                                            ))}
                                        </div>
                                    </>
                                )}
                            ...
                            <div className="flex items-center justify-center mb-6 text-gray-700 dark:text-gray-300"><div className="flex items-center mr-4">{renderStars(item.rating)}<span className="ml-2 text-xl font-bold">{item.rating ? item.rating.toFixed(1) : '0.0'}</span></div><div className="flex items-center"><MessageSquare size={20} className="mr-2 text-blue-500" /><span className="text-lg">{item.reviews || 0} รีวิว</span></div></div>
                            ...
                            <div className="mt-8">
                                <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4">ตำแหน่งบนแผนที่</h3>
                                <div className="text-center p-4 bg-gray-100 dark:bg-gray-700 rounded-lg">
                                    <p className="text-gray-600 dark:text-gray-300">ส่วนแสดงแผนที่ถูกปิดใช้งานชั่วคราว</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                {currentUser && (
                                  <button onClick={() => setShowReviewForm(!showReviewForm)} className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 transition-colors">{showReviewForm ? 'ยกเลิกการเขียนรีวิว' : 'เขียนรีวิวของคุณ'}</button>
                                )}
                                ...
                                <div className="space-y-0 -m-4">
                                    {reviews.map(review => (editingReview && editingReview.id === review.id 
                                        ? (
                                            <div key={`edit-${review.id}`} className="bg-blue-50 dark:bg-gray-900/50 border-l-4 border-blue-400 p-4 my-4 rounded-r-lg shadow-md animate-fade-in">
                                                <div className="flex justify-between items-center mb-4">
                                                    <h4 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center">
                                                        <Edit size={20} className="mr-2 text-blue-600 dark:text-blue-400" />
                                                        กำลังแก้ไขรีวิว
                                                    </h4>
                                                    <button onClick={handleCancelEdit} className="p-1 text-gray-500 hover:text-red-600 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors" title="ยกเลิกการแก้ไข">
                                                        <X size={20} />
                                                    </button>
                                                </div>
                                                <form onSubmit={handleUpdateReview} className="space-y-4">
                                                    <div>
                                                        <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">ให้คะแนนใหม่</label>
                                                        <StarRatingInput rating={editedRating} setRating={setEditedRating} />
                                                    </div>
                                                    <div>
                                                        <label htmlFor={`edit-comment-${review.id}`} className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">แก้ไขความคิดเห็น</label>
                                                        <textarea id={`edit-comment-${review.id}`} value={editedComment} onChange={(e) => setEditedComment(e.target.value)} rows="3" className="w-full p-2 border rounded-lg bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-500" />
                                                    </div>
                                                    <div>
                                                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">จัดการรูปภาพ</label>
                                                        {editedImages.length > 0 && (
                                                            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 mb-2">
                                                                {editedImages.map((image, index) => (
                                                                    <div key={index} className="relative group">
                                                                        <img src={image.type === 'new' ? URL.createObjectURL(image.data) : image.data} alt={`edit preview ${index}`} className="w-full h-20 object-cover rounded-lg" />
                                                                        <button type="button" onClick={() => handleRemoveEditedImage(index)} className="absolute top-0 right-0 -mt-2 -mr-2 bg-red-500 text-white rounded-full p-1 opacity-75 group-hover:opacity-100 transition-opacity"><X size={16} /></button>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        )}
                                                        {editedImages.length < 5 && (
                                                            <div className="mt-2">
                                                                <input type="file" multiple accept="image/*" onChange={handleEditImageChange} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-gray-100 dark:file:bg-gray-700 file:text-gray-700 dark:file:text-gray-200 hover:file:bg-gray-200" />
                                                            </div>
                                                        )}
                                                    </div>
                                                    <div className="flex space-x-2 pt-2">
                                                        <button type="submit" className="px-4 py-2 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 transition-colors">บันทึกการเปลี่ยนแปลง</button>
                                                        <button type="button" onClick={handleCancelEdit} className="px-4 py-2 bg-gray-200 text-gray-700 font-bold rounded-lg hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500 transition-colors">ยกเลิก</button>
                                                    </div>
                                                </form>
                                            </div>
                                        ) 
                                        : (
                                            <ReviewCard key={review.id} review={review} currentUser={currentUser} onReviewDeleted={handleDeleteReview} onEditClick={handleEditClick} />
                                        )
                                    ))}
                                </div>
                            </div>
                    ...
                    <aside className="lg:w-1/3 w-full mt-12 lg:mt-0 lg:sticky lg:top-8 self-start">
                        <div className="p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-xl ring-1 ring-black dark:ring-white ring-opacity-5 dark:ring-opacity-10">
                            <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-5">
                                {item.category ? `สถานที่อื่นในหมวดหมู่ "${item.category}"` : 'สถานที่แนะนำ'}
                            </h3>
                            {isSimilarLoading ? (
                                <p className="text-gray-500 dark:text-gray-400">กำลังค้นหาสถานที่...</p>
                            ) : similarPlaces.length > 0 ? (
                                <div className="space-y-4">
                                    {similarPlaces.map(place => (
                                        <div 
                                            key={place.id} 
                                            className="flex items-center gap-4 p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer transition-colors" 
                                            onClick={() => handleItemClick(place)}
                                        >
                                            <img 
                                                src={place.imageUrl || 'https://placehold.co/100x100/cccccc/333333?text=No+Image'} 
                                                alt={place.name} 
                                                className="w-16 h-16 object-cover rounded-lg flex-shrink-0 bg-gray-200 dark:bg-gray-600" 
                                            />
                                            <div className="overflow-hidden">
                                                <h5 className="font-bold text-gray-800 dark:text-gray-100 truncate">{place.name}</h5>
                                                <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{place.category}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <p className="text-gray-500 dark:text-gray-400 text-center p-4">ไม่พบสถานที่อื่นในหมวดหมู่นี้</p>
                            )}
                        </div>
                        ...
                        <div className="mt-8 p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-xl ring-1 ring-black dark:ring-white ring-opacity-5 dark:ring-opacity-10">
                            <div className="flex justify-between items-center mb-5">
                                <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-100 flex items-center">
                                    <Gift size={24} className="mr-3 text-amber-500" />
                                    ของขึ้นชื่อประจำที่นี่
                                </h3>
                                {currentUser && (
                                    <button 
                                        onClick={() => handleOpenProductModal(null)}
                                        className="flex items-center text-sm bg-blue-500 text-white font-semibold py-1 px-3 rounded-full hover:bg-blue-600 transition-colors"
                                    >
                            ...
                            {isLoadingProducts ? (
                                ...
                                <div className="space-y-4">
                                    {locationProducts.map(product => (
                                        <div key={product.id} className="group relative flex items-center gap-4 p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                            <img 
                                            ...
                                            {currentUser && (currentUser.id === product.user_id || currentUser.role === 'admin') && (
                                                <div className="absolute top-1 right-1 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => handleOpenProductModal(product)} className="p-1.5 bg-white/50 dark:bg-gray-600/50 rounded-full text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/50"><Edit size={14} /></button>
                                                    <button onClick={() => handleDeleteProduct(product.id)} className="p-1.5 bg-white/50 dark:bg-gray-600/50 rounded-full text-red-600 hover:bg-red-100 dark:hover:bg-red-900/50"><Trash2 size={14} /></button>
                                                </div>
                                            )}
                                        </div>
                                    ))}
                                ...
                                <p className="text-gray-500 dark:text-gray-400 text-center p-4">ยังไม่มีของขึ้นชื่อสำหรับสถานที่นี้</p>
                            )}
                        </div>
                    </aside>
