...
const NotificationItem = ({ notification, onNotificationClick }) => {
    ...
    return (
        <div onClick={handleItemClick} className={`w-full text-left flex items-start p-3 transition-colors duration-200 border-b dark:border-gray-700 ${notification.link ? 'hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer' : ''}`}>
            <div className="flex-shrink-0 relative">
                <img src={notification.userImage || 'https://placehold.co/48x48/e2e8f0/333333?text=🌍'} className="w-10 h-10 rounded-full object-cover shadow-sm"/>
            </div>
    ...
    return (
        <div className={`absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white dark:bg-gray-800 rounded-lg shadow-xl border dark:border-gray-700 transition-all duration-200 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
            <div className="flex justify-between items-center p-4 font-bold border-b dark:border-gray-700 text-gray-800 dark:text-white">
                การแจ้งเตือน
                <button onClick={onClose} className="text-gray-500 hover:text-gray-800 dark:hover:text-white"><X size={20} /></button>
            </div>
                ...
                <div className="hidden md:flex items-center gap-4">
                    ...
                    {currentUser && (
                        <div className="relative" ref={notificationPanelRef}>
                            <button onClick={toggleNotificationPanel} className="p-2 rounded-full text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                                <Bell size={22} />
                                {unreadCount > 0 && (
